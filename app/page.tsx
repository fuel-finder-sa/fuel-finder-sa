...
const [petrol95Price, setPetrol95Price] = useState("");
  ...
  const provinces=["All",...Array.from(new Set(stations.map(s=>s.province).filter(Boolean)))];

  const filtered=stations
    .filter(s=>{
      const txt=`${s.name||""} ${s.suburb||""} ${s.city||""} ${s.province||""}`.toLowerCase();
    ...
    .map(s=>{
      const distance=userLocation && typeof s.lat==="number" && typeof s.lng==="number"
  ...
  return(
    <main style={{
      ...
      <section style={{
        ...
        <p style={{color:"#cbd5e1",marginTop:8}}>Find cheapest & safest fuel stops.</p>
      ...
      {showAddForm&&(
        <Modal>
          ...
          <input placeholder="Station Name" style={inputStyle}
            onChange={e=>setNewStation({...newStation,name:e.target.value})}/>
          <input placeholder="Location / suburb / town" style={inputStyle}
            onChange={e=>setNewStation({...newStation,location:e.target.value})}/>
          <input placeholder="Diesel 50ppm price" style={inputStyle}
            onChange={e=>setNewStation({...newStation,diesel50:e.target.value})}/>
          <input placeholder="Petrol 93 price" style={inputStyle}
            onChange={e=>setNewStation({...newStation,petrol93:e.target.value})}/>
          <input placeholder="Petrol 95 price" style={inputStyle}
            onChange={e=>setNewStation({...newStation,petrol95:e.target.value})}/>
          <input placeholder="Features: ATM, toilets, truck wash, food court" style={inputStyle}
            onChange={e=>setNewStation({...newStation,features:e.target.value})}/>

          <div style={{display:"flex",gap:10}}>
            <button onClick={submitStation} style={{...actionButton,background:"#059669"}}>Submit</button>
            <button onClick={()=>setShowAddForm(false)} style={{...actionButton,background:"#dc2626"}}>Cancel</button>
          </div>
        </Modal>
      )}
      ...
      {showReportForm&&selectedStation&&(
        <Modal>
          ...
          <div style={{display:"flex",gap:10}}>
            <button onClick={submitReport} style={{...actionButton,background:"#dc2626"}}>Submit Report</button>
            <button onClick={()=>setShowReportForm(false)} style={{...actionButton,background:"#111827"}}>Cancel</button>
          </div>
        </Modal>
      )}
      ...
      {showUpdateForm&&selectedStation&&(
        <Modal>
          ...
          <div style={{display:"flex",gap:10}}>
            <button onClick={submitPriceUpdate} style={{...actionButton,background:"#059669"}}>Submit Price</button>
            <button onClick={()=>setShowUpdateForm(false)} style={{...actionButton,background:"#111827"}}>Cancel</button>
          </div>
        </Modal>
      )}
      ...
      <section style={{
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={()=>setTruckMode(!truckMode)}
            style={{...actionButton,background:truckMode?"#22c55e":"#e5e7eb",color:truckMode?"white":"#111827"}}>
            🚛 Truck Mode
          </button>

          <button onClick={()=>setOpen24(!open24)}
            style={{...actionButton,background:open24?"#22c55e":"#e5e7eb",color:open24?"white":"#111827"}}>
            🕒 Open 24h
          </button>
        </div>
      </section>
      ...
      {filtered[0]&&(
        <section style={{
          ...
          <div style={{fontSize:22,fontWeight:900,marginTop:8}}>{filtered[0].name}</div>
          <div style={{marginTop:8}}>💸 Diesel: <b>R{filtered[0].diesel50}</b></div>
          {filtered[0].distance!==null&&(
            <div style={{marginTop:4}}>📍 {filtered[0].distance.toFixed(1)} km away</div>
          )}
        </section>
      )}
      ...
      {filtered.map((s,index)=>(
        <article key={s.id} style={{
          ...
          <div style={{
            ...
            <button
              onClick={()=>window.open(`https://www.google.com/maps?q=${s.lat},${s.lng}`,"_blank")}
